#pragma once

#include "Helpers/Common.h" // Assumes this defines 'struct vec3 {float x, y, z;}'
#include <string>

// Forward declaration
class TextureLoader;

// --- MD2 Data Structures ---
// (Restored full definitions and changed vec3_t to vec3)

/* MD2 header */
struct md2_header_t
{
    int ident;
    int version;
    int skinwidth;
    int skinheight;
    int framesize;
    int num_skins;
    int num_vertices;
    int num_st;
    int num_tris;
    int num_glcmds;
    int num_frames;
    int offset_skins;
    int offset_st;
    int offset_tris;
    int offset_frames;
    int offset_glcmds;
    int offset_end;
};

/* Texture name */
struct md2_skin_t
{
    char name[64];
};

/* Texture coords */
struct md2_texCoord_t
{
    short s;
    short t;
};

/* Triangle info */
struct md2_triangle_t
{
    unsigned short vertex[3];
    unsigned short st[3];
};

/* Compressed vertex */
struct md2_vertex_t
{
    unsigned char v[3];
    unsigned char normalIndex;
};

/* Model frame */
struct md2_frame_t
{
    vec3 scale;     // <-- CHANGED from vec3_t
    vec3 translate; // <-- CHANGED from vec3_t
    char name[16];
    struct md2_vertex_t *verts;
};

/* GL command packet */
struct md2_glcmd_t
{
    float s;
    float t;
    int index;
};

/* MD2 model structure */
struct md2_model_t
{
    struct md2_header_t header;
    struct md2_skin_t *skins;
    struct md2_texCoord_t *texcoords;
    struct md2_triangle_t *triangles;
    struct md2_frame_t *frames;
    int *glcmds;
    GLuint tex_id;
};


// --- Loader Class ---

class _3DModelLoader
{
    public:
        _3DModelLoader();
        virtual ~_3DModelLoader();

        void initModel(const char *filename);
        void Draw();

        // --- New Avatar Methods ---
        void update(float dt); // Handles animation logic
        void stand();          // Switch to "stand" animation
        void walk();           // Switch to "walk" animation

        // Position/Rotation methods
        void translate(double dx, double dy, double dz);

        double rotateX;
        double rotateY;
        double rotateZ;
        double scale;

    private:
        // --- Private Methods ---
        int ReadMD2Model (const char *filename, struct md2_model_t *mdl);
        void RenderFrameItpWithGLCmds (int n, float interp, const struct md2_model_t *mdl);
        void Animate (int start, int end, int *frame, float *interp);
        void FreeModel (struct md2_model_t *mdl);
        void setAnimation(int start, int end);

        // --- Private Members ---
        TextureLoader* myTex;

        // This is no longer an "incomplete type"
        struct md2_model_t md2file;

        // --- Animation State ---
        int animStandStart, animStandEnd;
        int animWalkStart, animWalkEnd;

        int currentAnimStart;
        int currentAnimEnd;
        int currentFrame;
        float interp;
        double lastTime;

        // --- Model Position ---
        vec3 pos;
};
